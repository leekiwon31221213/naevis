(function(window) {
    ...
    function render(timestamp) {
        ...
        // Object-fit: cover처럼 이미지 그리기
        const imageAspectRatio = _imageData.width / _imageData.height;
        const canvasAspectRatio = width / height;
        let renderableWidth, renderableHeight, xStart, yStart;

        // 캔버스 비율에 따라 이미지 크기 조정
        if (imageAspectRatio < canvasAspectRatio) {
            renderableHeight = height;
            renderableWidth = _imageData.width * (height / _imageData.height);
            xStart = (width - renderableWidth) / 2;
            yStart = 0;
        } else if (imageAspectRatio > canvasAspectRatio) {
            renderableWidth = width;
            renderableHeight = _imageData.height * (width / _imageData.width);
            xStart = 0;
        ...
        _context.drawImage(_imageData, xStart, yStart, renderableWidth, renderableHeight);
